...
const SendMsgInput = () => {
	...
	const params = useParams<{id:string}>(); //to get receiver id on client side
	...
	const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			console.log("SendMsgInput Error: ",error);
		}finally{
			setIsLoading(false);
		}
	}
	return (
		<div className='flex gap-2 items-center py-1'>
			...
			<form onSubmit={handleSubmit} className='flex-1 flex  items-center gap-1 bg-sigBackgroundSecondaryHover rounded-full border   border-sigColorBgBorder'>
				<Input
